...
import BuyingOptions from './BuyingOpption';
...
const ProductsPage: React.FC = () => {
  ...
  const filteredProducts = selectedCategory === 'All'
    ? [
        products.find(product => product.category === "Men's Clothing"),
        products.find(product => product.category === "Women's Clothing"),
        products.find(product => product.category === 'Jewelry'),
        products.find(product => product.category === 'Watches'),
        products.find(product => product.category === "Women's Shoes"),
        products.find(product => product.category === "Men's Shoes"),
      ].filter((product): product is Product => product !== undefined)
    : products.filter(product => product.category === selectedCategory);

  // Slice the products to only show a limited number initially
  ...
  return (
    <div className="min-h-screen bg-black text-white flex flex-col items-center py-12 px-4 sm:px-6 lg:px-8">
      ...
      <div className="mb-4 w-full max-w-4xl">
        <label htmlFor="category" className="block text-lg font-semibold text-white mt-4">Filter by Category:</label>
        <select
          id="category"
          className="mt-2 p-2 w-full sm:w-1/2 md:w-1/4 lg:w-1/4 border border-white bg-black text-white rounded"
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 w-full max-w-6xl">
        {visibleProducts.map((product) => (
          <div key={product.id} className="border p-4 rounded-md bg-gray-800 text-white">
            {/* Product image and category name */}
            <div className="relative w-full h-48 mb-4">
              <Link href={`/product/${product.id}`} className="w-full h-full rounded block">
                <img
                  src={product.image}
                  alt={product.name}
            ...
            {/* Buying options (custom component) */}
      ...
      <Link href="/Shop" legacyBehavior>
        <button className="mt-8 px-6 py-3 border-2 border-yellow-500 text-yellow-500 font-semibold rounded-md hover:bg-yellow-500 hover:text-white transition-all">
          Show More
        </button>
      </Link>
    </div>
