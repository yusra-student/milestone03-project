...
const ProductsPage = () => {
  ...
  return (
    <div className="min-h-screen bg-white text-black flex flex-col items-center py-12 px-4 sm:px-6 lg:px-8">
      <div className="w-full h-64 sm:h-80 md:h-80 lg:h-96 bg-cover bg-center relative" style={{ backgroundImage: "url('/shop.png')" }}>
      </div>
      <div className="mb-4 w-full max-w-4xl">
        <label htmlFor="category" className="block text-lg font-semibold text-black mt-4">Filter by Category:</label>
        <select
          id="category"
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-3 xl:grid-cols-4 gap-4 w-full max-w-6xl">
        {filteredProducts.map((product) => (
          <div key={product.id} className="border p-4 rounded-md bg-gray-800 text-white relative">
            <a href={`/product/${product.id}`} className="w-full h-48 mb-4 rounded block">
              <img
                src={product.image}
                alt={product.name}
...
<div className="absolute top-2 left-2 bg-black bg-opacity-50 text-white text-sm px-2 py-1 rounded">
  {product.category}
            ...
            <a
              ...
              className="flex items-center justify-center space-x-2 bg-green-500 text-white py-2 px-4 rounded-md hover:bg-green-600 transition-all"
            >
